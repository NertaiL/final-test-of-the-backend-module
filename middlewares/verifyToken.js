...
const validateToken = async (token) => {
  //aca llega el token  y aplicamos el jwt.verify le pasamos el token parametrizado y la firma que tenemos guardada en nuestro .env
  try {
...
const validateHeaders = (req) => {
  ...
  if (!req.header("Authorization")) {
    //si no tiene la authorization entonces
    throw createError("auth_03", "token no encontrado"); //le tiramos un error que el token no ha sido encontrado
  }
};

const createError = (code, message) => {
  //aqui cree una funcion de errores para poder ocupar el codigo auth_03 etc.
